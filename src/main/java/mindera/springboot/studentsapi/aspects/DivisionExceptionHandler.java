...
public class DivisionExceptionHandler {
    ...
    public ResponseEntity<String> sqlError(Exception ex) {
        logger.error("Known Exception: " + ex);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("unexpeted error about sql");
    }


    @ExceptionHandler(value = { BadDenominatorDivision.class})
    public ResponseEntity<String> baddivide(Exception ex) {
        logger.error("Known Exception: " + ex);
